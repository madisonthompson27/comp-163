...
def decoded_message(secondMsg, finalMsg=""):
    #for loop converts values to the proper dict in one line, k for key and v for value. will error if value not in dict. 
    decodeDic = {v : k for k, v in encodeDic.items()}
    #print(decodeDic) #testing to makes sure dic is inverted
    
    #using numeric iteration so that strings are protected from accidental mutation
    count = 0
    char = secondMsg[count]
    
    #if the character from the string is present add the decoding value to finalMsg
        #some bugs were present with this statement, so for safety the test version is currently running. 
    if char in decodeDic.keys():
        finalMsg = decodeDic.get(char)
